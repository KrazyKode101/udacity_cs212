...
def set_anchors(row, j, board):
    ...
    for (i, sq) in enumerate(row[1:-1], 1):
        neighborlist = (N,S,_,_) = neighbors(board, i, j)
...
def all_plays(hand, board):
    ...
    vplays = horizontal_plays(hand, list(transpose(board))) # set of ((j, i), word)
    ###Your code here.
    """
    results = set()
    for play in hplays:
    ...
    for play in vplays:
        results.add(((play[0][1],play[0][0]),DOWN,play[1]))
    return results
    """
    return (set( (((i,j),ACROSS,word) for ((i,j),word) in hplays ))  | set( (((j,i),DOWN,word) for ((i,j),word) in vplays )))

print(all_plays("CHADEINRSTOYLD", list(lesson6_quiz6.a_board())))