...
def boggle_words(board, minlength=3):
    ...
    for (i,ch) in enumerate(board):
        if ch != '|':
            find_words(board, minlength, i, ch, visited, results)

    return results

def find_words(board, minlength, i, prefix, visited, results):
    if len(prefix) >= minlength and prefix in WORDS:
        results.add(prefix)
    ...
    visited[i] = 1
    n = neighbors(i, size(board))
    for e in n:
        if board[e] != '|' and visited[e] == 0:
            find_words(board, minlength, e, prefix + board[e], visited, results)
