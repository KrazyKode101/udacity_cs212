...
def subway(**lines):
    ...
    for linename,stations in lines.items():
        ...
        linelen = len(stations)
        for index,station in enumerate(stations):
            neighbors = system.get(station,{})
            if index != linelen-1:
                neighbors[stations[index+1]] = linename
            if index !=0:
                neighbors[stations[index-1]] = linename
            system[station] = neighbors
    return system
...
def ride(here, there, system=boston):
    ...
    ## your code here
    successors = lambda state : system.get(state,{})
...
def longest_ride(system):
    ...
    longest_ride_path = []
    for here,there in itertools.combinations(stations,2):
        path = ride(here,there,system)
        longest_ride_path = max(path_states(path),longest_ride_path,key=len)

    return longest_ride_path
