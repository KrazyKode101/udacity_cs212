...
def natalie(words):
    ...
    max_p_word = None
    max_p_score = None
    for each in itertools.permutations(words,2):
        word1 = each[0]
        word2 = each[1]
        try:
            start,mid,end = get_p_word(word1,word2)
            p_score = get_p_score(start,mid,end)
            if max_p_score == None or p_score > max_p_score:
                max_p_score = p_score
                max_p_word = start+mid+end
        except Exception as e:
            continue
    
    return max_p_word

def get_p_word(word1,word2):
    m = i = len(word1)-1
    n = j = len(word2)-2
    ...
    if i == -1 or n == -1:
        raise Exception("no p_word found")
    else:
        return word1[0:i+1],word1[i+1:],word2[n+1:]

def get_p_score(start,mid,end):
    total_len = len(start)+len(mid)+len(end)
    return total_len - abs(len(start) - total_len/4) - abs(len(mid) - total_len/2) - abs(len(end) - total_len/4)
