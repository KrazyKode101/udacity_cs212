...
POINTS.update(dict.fromkeys(list(string.ascii_lowercase),0))
...
def is_empty(sq):
    ...
    return sq  == '.' or isinstance(sq, set) 

def replace_and_get(p,SET):
    result = set()
    if p.find('_') != -1:
        for each in itertools.product( (letters for _ in range(p.count('_'))) ):
            s = p[:]
            i = 0
            c = s.count('_')
            for _ in range(c):
                s = s.replace('_',each[i],1)
                i += 1
            if s.upper() in SET:
                result.add(s)
    else:
...
def add_suffixes(hand, pre, start, row, results, anchored=True):
    ...
    i = start + len(pre)
    words = replace_and_get(pre, WORDS)
    if len(words)>0 and anchored and not is_letter(row[i]):
        for p in words:
            results.add((start, p))

    prefixes = replace_and_get(pre, PREFIXES)
    if len(prefixes)>0:
        sq = row[i]
        if is_letter(sq):
...
def find_prefixes(hand, row, i, maxsize, pre='', results=None):
    ...
    words = replace_and_get(pre, WORDS)
    prefixes = replace_and_get(pre, PREFIXES)
    if len(words)>0 or len(prefixes)>0: results.union(prefixes)
    if len(pre) < maxsize and len(prefixes)>0:
        possibilities = row[i] if isinstance(row[i],set) else ANY
        for L in hand:
            if L in possibilities:
                find_prefixes(hand.replace(L, '', 1), row, i+1, maxsize, pre+L, results)
    return results
...
def legal_prefix(i, row):
    ...
    while is_empty(row[s-1]) and not is_letter(row[s-2]):
        s -= 1
...
def set_anchors(row, j, board):
    ...
    for (i, sq) in enumerate(row[1:-1], 1):
        ...
        # Anchors are squares adjacent to a letter.
        if is_empty(sq) and any(map(is_letter, neighborlist)):
...
ACROSS, DOWN = (0, 1), (1, 0) # Directions that words can go
...
def all_plays(hand, board):
    ...
    vplays = horizontal_plays(hand, list(transpose(board)))
...
def show(board):
    ...
    for (i,row) in enumerate(board):
        for (j,col) in enumerate(row):
            print(col,end=" ") if (is_letter(col) or col == '|') else print(BONUS[i][j],end=" ")
        print()
    return None
...
BONUS = list(bonus_board())
DW, TW, DL, TL = '23:;'

def test():
    print(best_play('ABCEHKN', list(a_board())))
    print(best_play('_BCEHKN', list(a_board())))
    print(best_play('__CEHKN', list(a_board())))
